...
from mmcv.ops import nms_bev as nms_gpu, nms_normal_bev as nms_normal_gpu
...
class FullySparseBboxHead(BaseModule):
        ...
        if pos_inds.any() == 0:
            ...
            if self.with_corner_loss:
                ...
                pred_boxes3d[..., 0:3] = rotation_3d_in_axis(
                    ...
                    -pos_rois_rotation,   # NOTE coordination refactor
                    axis=2,
                    clockwise=True).squeeze(1)
                ...
                if self.train_cfg.get('corner_loss_only_car', True):
                    ...
                    car_type_index = self.train_cfg['class_names'].index('vehicle')
    ...
    def _get_target_single(self, pos_bboxes, pos_gt_bboxes, ious, pos_labels, cfg):
        ...
        if reg_mask.bool().any():
            ...
            pos_gt_bboxes_ct[..., 0:3] = rotation_3d_in_axis(
                ...
                #-(roi_ry + np.pi / 2),
                roi_ry,      # NOTE  for coordination refactor
                axis=2,
                clockwise=True).squeeze(1)
    ...
    def get_bboxes(
        ...
        rcnn_boxes3d[..., 0:3] = rotation_3d_in_axis(
            #rcnn_boxes3d[..., 0:3].unsqueeze(1), (roi_ry + np.pi / 2),   # NOTE coordination refactor
            rcnn_boxes3d[..., 0:3].unsqueeze(1), -roi_ry,
            axis=2, clockwise=True).squeeze(1)
        ...
        for batch_id in range(batch_size):
            ...
            # selected_scores = class_pred[batch_id][selected]   # stage1 score
