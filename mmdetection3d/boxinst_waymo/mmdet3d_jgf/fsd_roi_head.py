...
class GroupCorrectionHead(Base3DRoIHead):
    ...
    def _assign_and_sample(self, proposal_list, gt_bboxes_3d, gt_labels_3d):
        ...
        for batch_idx in range(len(proposal_list)):
            ...
            proposal_ious.append(assign_result.max_overlaps.cpu().numpy())
            sampled_ious.append(sampling_result.iou.cpu().numpy())
            # # sampled_pos_iou.append(sampling_result.iou.cpu().numpy()[sampling_result.pos_inds.cpu().numpy()])
            # # sampled_neg_iou.append(sampling_result.iou.cpu().numpy()[sampling_result.neg_inds.cpu().numpy()])
        
        histogram_proposal_iou = np.concatenate(proposal_ious)
        histogram_sampled_iou = np.concatenate(sampled_ious)
        # histogram_sampled_pos_iou = np.concatenate(sampled_pos_iou)
        # histogram_sampled_neg_iou = np.concatenate(sampled_neg_iou)

        gol.set_tensorboard_value("histogram_proposal_iou", histogram_proposal_iou)
        gol.set_tensorboard_value("histogram_sampled_iou", histogram_sampled_iou)
        # gol.set_tensorboard_value("histogram_sampled_pos_iou", histogram_sampled_pos_iou)
        # gol.set_tensorboard_value("histogram_sampled_neg_iou", histogram_sampled_neg_iou)
