...
class CartesianVoxelization(Voxelization):
    def __init__(self,
                 cylinder_partition,
                 cylinder_range,
                 max_num_points,
                 ...
                 clamp_input=False,
                 deterministic=True):
        assert (max_num_points == -1) or (max_voxels == -1) # dynamic voxelization
        ...
        super(CartesianVoxelization, self).__init__(
            voxel_size=voxel_size.tolist(),
            point_cloud_range=cylinder_range.tolist(),
            max_num_points=max_num_points,
            max_voxels=max_voxels,
    ...
    def forward(self, input):
        output = super(CartesianVoxelization, self).forward(input)
        return input, output
