...
from .voxelize import Cylinderization, CartesianVoxelization
# import time
...
class Cylinder3D(Base3DSegmentor):
    def __init__(
        ...
        if pts_voxel_layer:
            type = pts_voxel_layer.get('type', None)
            assert type in ['Cylinder', 'Cartesian']
            pts_voxel_layer.pop('type')
            if type == 'Cylinder':
                self.pts_voxel_layer = Cylinderization(**pts_voxel_layer)
            elif type == 'Cartesian':
                self.pts_voxel_layer = CartesianVoxelization(**pts_voxel_layer)
    ...
    def simple_test(self, points, img_metas, img=None, rescale=False,
        ...
        assert len(points) == 1
        num_pts, in_chs = points[0].shape
        cur_frame_mask = points[0].new_full((num_pts,), True, dtype=torch.bool)
        if (in_chs == 5):
            cur_frame_mask = (points[0][:, 4] < 0.01)
        
        results_list = self.pts_seg_head.simple_test(
            x, self.coors, img_metas, rescale, pts_semantic_mask, cur_frame_mask)
        # print('----------infer time is----------', (time.time() - start))
        return results_list
