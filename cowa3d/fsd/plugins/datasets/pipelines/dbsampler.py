...
class DataBaseSamplerWithSeg(object):
    def __init__(self,
                 info_path,
                 rate,
                 filter,
        ...
        self.cat2seglabel = cat2seglabel
    ...
    def sample_all(self, input_dict, sample_2d=False):
        ...
        gt_pts_semantic_mask = input_dict.get('pts_semantic_mask', None)
        ...
        if len(sampled) > 0:
            ...
            for info in sampled:
                ...
                s_pts_res = self.points_loader(info)
                s_points = s_pts_res['points']
                s_sweep_ind = s_pts_res.get('sweep_ind', None)

                count += 1

                s_points_list.append(s_points)
                s_sweep_ind_list.append(s_sweep_ind)

                if gt_pts_semantic_mask is not None:
                    semantic_label = self.cat2seglabel[info['name']]
                    # if s_sweep_ind is not None:
                    #     semantic_pts_num = (s_sweep_ind == 0).sum()
                    semantic_pts_num = s_points.tensor.shape[0]
                    s_semantic_mask = np.ones(
                                        semantic_pts_num, 
                                        dtype=gt_pts_semantic_mask.dtype) * semantic_label
                    s_semantic_mask_list.append(s_semantic_mask)
            ...
            if s_sweep_ind_list[0] is not None:
                ret['sweep_ind'] = np.concatenate(s_sweep_ind_list)
            if gt_pts_semantic_mask is not None:
                ret['pts_semantic_mask'] = np.concatenate(s_semantic_mask_list)
