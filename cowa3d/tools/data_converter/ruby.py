...
def process_ruby_single(idx, pcd, ann, root_path, bin_path, gt_bin_path=None):
    xyzi = read_pcd(pcd)
    xyzi[..., -1] /= 255.0
...
if __name__ == '__main__':
    ...
    isval = lambda x: x.startswith('ruby_ruby002_baoshenlu')

    if osp.isdir(osp.join(data_root, 'kitti_format')):
        shutil.rmtree(osp.join(data_root, 'kitti_format'))
    os.makedirs(osp.join(data_root, 'kitti_format/training/ruby'),
                exist_ok=True)
    os.makedirs(osp.join(data_root, 'kitti_format/ImageSets'), exist_ok=True)
    os.makedirs(osp.join(data_root, 'kitti_format/ruby_gt_database'),
                exist_ok=True)
    ...
    for idx, d_id in enumerate(track_iter_progress(ids)):
        frame_anno, frame_gt = process_ruby_single(
            idx, pcd_ids[d_id], json_ids[d_id], f'{data_root}/kitti_format',
            bin_path='training/ruby', gt_bin_path='ruby_gt_database')
    ...
    with open(osp.join(data_root, 'kitti_format/ruby_infos_trainval.pkl'),
              'wb') as f:
        pickle.dump(annos, f)
    with open(osp.join(data_root, 'kitti_format/ruby_infos_train.pkl'),
              'wb') as f:
        pickle.dump(annos_train_only, f)
    with open(osp.join(data_root, 'kitti_format/ruby_infos_val.pkl'),
              'wb') as f:
        pickle.dump(annos_val_only, f)

    with open(osp.join(data_root, 'kitti_format/ruby_dbinfos_trainval.pkl'),
              'wb') as f:
        pickle.dump(gts, f)
    with open(osp.join(data_root, 'kitti_format/ruby_dbinfos_train.pkl'),
              'wb') as f:
