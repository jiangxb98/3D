...
def create_kitti_info_file(data_path,
    ...
    minival_img_ids = _read_imageset_file(str(imageset_folder / 'minival.txt'))
    ...
    kitti_infos_maxitrain = []
    kitti_infos_minival = []
    for info in kitti_infos_train + kitti_infos_val:
        if int(info['image']['image_idx']) in minival_img_ids:
            kitti_infos_minival.append(info)
        else:
            kitti_infos_maxitrain.append(info)

    filename = save_path / f'{pkl_prefix}_infos_maxitrain.pkl'
    print(f'Kitti info maxitrain file is saved to {filename}')
    mmcv.dump(kitti_infos_maxitrain, filename)

    filename = save_path / f'{pkl_prefix}_infos_minival.pkl'
    print(f'Kitti info minival file is saved to {filename}')
    mmcv.dump(kitti_infos_minival, filename)
...
if __name__ == '__main__':
    ...
    create_groundtruth_database(
        ...
        root_path,
        info_prefix,
        f'{outdir}/{info_prefix}_infos_train.pkl',
        relative_path=False,
        with_mask=False)
    ...
    create_groundtruth_database(
        ...
        root_path,
        info_prefix,
        f'{outdir}/{info_prefix}_infos_trainval.pkl',
        database_save_path=f'{root_path}/{info_prefix}_gt_database_trainval',
        db_info_save_path=f'{root_path}/{info_prefix}_dbinfos_trainval.pkl',
        relative_path=False,
        with_mask=False)
    ...
    create_groundtruth_database(
        ...
        root_path,
        info_prefix,
        f'{outdir}/{info_prefix}_infos_maxitrain.pkl',
        database_save_path=f'{root_path}/{info_prefix}_gt_database_maxitrain',
        db_info_save_path=f'{root_path}/{info_prefix}_dbinfos_maxitrain.pkl',
        relative_path=False,
        with_mask=False)
